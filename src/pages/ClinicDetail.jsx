...
export default function ClinicDetail() {
  ...
  if (!clinic) {
    return (
      <div className="max-w-[430px] mx-auto p-8 text-center text-gray-400">
        クリニックが見つかりません
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      {/* Hero banner */}
      <div className="bg-gradient-to-br from-teal-700 to-teal-900 max-w-[430px] mx-auto px-4 py-5">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <h1 className="text-white text-lg font-bold leading-tight">{clinic.name}</h1>
            <p className="text-teal-200 text-xs mt-1">{clinic.director} 院長　{clinic.directorTitle}</p>
          </div>
          <div className="text-right flex-shrink-0 ml-3">
            <div className="text-white font-bold text-lg">{clinic.priceRange}</div>
            <div className="text-teal-200 text-[10px]">最低料金目安</div>
          </div>
        </div>
        <div className="flex items-center gap-1.5 mt-3">
          <StarRating rating={clinic.rating} size="md" />
          <span className="text-amber-300 font-bold text-sm">{clinic.rating}</span>
          <span className="text-teal-200 text-xs">（{clinic.reviewCount}件の口コミ）</span>
        </div>
      </div>
      ...
      <div className="max-w-[430px] mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-sm mt-4 p-4">
          ...
          <div className="mt-3 p-3 bg-teal-50 rounded-lg border border-teal-100">
            <p className="text-teal-800 text-xs leading-relaxed">{clinic.description}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm mt-4 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded">相場は市場平均値</span>
          </div>
          <div className="space-y-3">
            {Object.entries(clinic.prices).map(([key, item]) => (
              <div key={key} className="border border-gray-100 rounded-lg p-3">
                <div className="flex items-start justify-between mb-1">
                  <span className="text-gray-700 text-xs font-medium flex-1 pr-2">{item.name}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-teal-700 font-bold text-base">{formatPrice(item.price)}</span>
                    <PriceDiff price={item.price} market={item.market} />
                  </div>
                  <div className="text-right">
                    <div className="text-gray-400 text-[10px]">相場: {formatPrice(item.market)}</div>
                  </div>
                </div>
          ...
          <div className="mt-3 p-2.5 bg-amber-50 rounded-lg border border-amber-100">
            <p className="text-amber-700 text-[10px] leading-relaxed">
              ※ 掲載料金は第三者審査済みの税込み参考価格です。治療内容・難易度により変動する場合があります。詳細は無料相談でご確認ください。
            </p>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-40">
        <div className="max-w-[430px] mx-auto p-3 space-y-2">
          <button
